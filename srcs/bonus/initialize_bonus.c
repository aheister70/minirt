...
/*   Updated: 2021/06/25 16:11:52 by aheister      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
** INIT_LIGHT - Assigns l value from RT file to struct s_light (multi)
** INIT_CAMERA - Assigns values from RT file to struct s_camera
...
void	init_scene(char	*line, t_scene *scene)
{
	...
	if (object[0] != NULL)
	{
		...
		else if (ft_strncmp(object[0], "tr\0", 3) == 0)
			init_triangle(object, scene);
		else
...
void	init_light(char **object, t_scene *scene)
{
	...
	if (light->brightness < 0.0 || light->brightness > 1.0)
		error_handling("Value of light brightness is invalid", object, scene);
